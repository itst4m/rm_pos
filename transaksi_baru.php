...
<script type="text/javascript">
		$(document).ready(function(){
			if ($.trim($('#contenth').text())=="") {
				...
				$('#prosestran').attr("title","tambahkan makanan terlebih dahulu");
	...
	<div class="padding">
			...
			<div class="padding">
				...
				<form class="form-input" method="post" action="handler.php?action=tambah_tempo" style="padding-top: 30px;">
					...
					<select style="width: 372px;cursor: pointer;" required="required" name="id_barang">
						<?php
						$data=$root->con->query("select * from makanan");
		...
		<div class="bgwhite">
			...
			<tbody id="contenth">
				...
				$data=$root->con->query("select makanan.nama_barang,tempo.id_subtransaksi,tempo.id_barang,tempo.jumlah_beli,tempo.total_harga from tempo inner join makanan on makanan.id_barang=tempo.id_barang where trx='$trx'");
